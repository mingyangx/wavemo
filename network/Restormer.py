...
# The code is from https://github.com/leftthomas/Restormer.
class Restormer(nn.Module):
    def __init__(self, num_blocks=[4, 6, 6, 8], num_heads=[1, 2, 4, 8], channels=[48, 96, 192, 384], num_refinement=4,
                 expansion_factor=2.66, img_ch=3, output_ch=3, residual=True):
        ...
        # originally img_ch = 3, but here it refers to the number of SLMs.
        self.embed_conv = nn.Conv2d(img_ch, channels[0], kernel_size=3, padding=1, bias=False)
        ...
        self.output = nn.Conv2d(channels[1], output_ch, kernel_size=3, padding=1, bias=False)

    def forward(self, x):
        # x input here in our case in [B, Nframe, 1, H, W]
        # We will need to  squeeze the color channel since we are using grayscale images
        ...
        if self.residual:
            out = self.output(fr) + x_zero
        else:
            out = self.output(fr) 

        # out = self.output(fr) + x  # The original code is already using residual learning
