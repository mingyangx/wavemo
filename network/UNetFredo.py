...
class FredoUNet(nn.Module):
    ...
    def final_block(self, in_channels, out_channels, mid_channel, kernel_size=3, final_sigmoid=False):
        if final_sigmoid:
            block = torch.nn.Sequential(
    ...
    def __init__(self, in_channel, out_channel, nframes=8, residual=False, sigmoid=False, permutation_invariant=True):
        super(FredoUNet, self).__init__()
        ...
        self.permutation_invariant = permutation_invariant # if set to False, this network is no longer permutation invariant!
        ...
        self.final_conv_instead_of_pooling = self.conv_elu(in_channels=self.nframes, out_channels=out_channel, kernel_size= 3)
        self.final_layer  = self.final_block(in_channels=96, out_channels = out_channel, mid_channel= 64, final_sigmoid=self.sigmoid)      #id 40, 41
    ...
    def forward(self, x, output_many_frames=False):
        ...
        if self.permutation_invariant:
            ...
            # info(features9, 'before final layer, feature')
            # info(max_global_features9, 'before final layer, maxpool')
            # info(self.final_layer(features9.squeeze(0)), 'new output')
            ...
            if output_many_frames:
                print('Not implemented')  
                exit()
            else:
                # info(features9, 'features9')
                global_features9 = self.final_layer(features9.squeeze(0))
                # info(global_features9, 'global_features9')
                final_layer = self.final_conv_instead_of_pooling(global_features9.permute(1, 0, 2, 3))
                # info(final_layer, 'final_layer')
                # exit()
        ...
        if self.residual:
            ...
            if not output_many_frames:
                res =  x_mean - final_layer
            else:
                res =  x.squeeze(0) - final_layer
        else:
            res =  final_layer
